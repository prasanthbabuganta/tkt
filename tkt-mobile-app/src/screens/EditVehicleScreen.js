...
import { updateVehicle, clearSuccess, clearError } from '../slices/vehicleSlice';
...
const EditVehicleScreen = ({ route, navigation }) => {
  ...
  const [ownerMobile, setOwnerMobile] = useState(vehicle.ownerMobile || '');
  ...
  const [existingCarImageUrl, setExistingCarImageUrl] = useState(vehicle.carImageUrl || null);
  const [existingKeyImageUrl, setExistingKeyImageUrl] = useState(vehicle.keyImageUrl || null);
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        if (imageType === 'car') {
          ...
          setExistingCarImageUrl(null);
        } else {
          setKeyImage(selectedImage);
          setExistingKeyImageUrl(null);
        }
      }
    } catch (error) {
        ...
        <View style={styles.form}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TouchableOpacity
              ...
              disabled={loading}
            >
              {carImage || existingCarImageUrl ? (
                <View style={styles.imagePreviewContainer}>
                  <Image
                    source={{ uri: carImage ? carImage.uri : existingCarImageUrl }}
          ...
          <View style={styles.inputGroup}>
            ...
            <TouchableOpacity
              ...
              disabled={loading}
            >
              {keyImage || existingKeyImageUrl ? (
                <View style={styles.imagePreviewContainer}>
                  <Image
                    source={{ uri: keyImage ? keyImage.uri : existingKeyImageUrl }}
                    style={styles.imagePreview}
                  />
                  <TouchableOpacity
                    style={styles.removeImageButton}
